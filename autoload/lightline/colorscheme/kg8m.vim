let s:true_black = [ '#000000', 0 ]
let s:black      = [ '#121212', 233 ]
let s:dark_gray  = [ '#1c1c1c', 234 ]
let s:gray       = [ '#262626', 235 ]
let s:light_gray = [ '#666666', 242 ]
let s:white      = [ '#ffffff', 255 ]
let s:sand       = [ '#e6db74', 144 ]
let s:blue       = [ '#66d9ef', 81  ]
let s:green      = [ '#a6e22e', 118 ]
let s:orange     = [ '#fd971f', 208 ]
let s:red        = [ '#f92672', 161 ]

let s:p = {'normal': {}, 'inactive': {}, 'insert': {}, 'replace': {}, 'visual': {}, 'tabline': {}}
let s:p.normal.left     = [ [ s:black, s:sand ], [ s:white, s:gray ] ]
let s:p.normal.middle   = [ [ s:white, s:dark_gray ] ]
let s:p.normal.right    = [ [ s:black, s:sand ], [ s:white, s:gray ] ]
let s:p.normal.error    = [ [ s:white, s:red ] ]
let s:p.normal.warning  = [ [ s:white, s:red ] ]
let s:p.inactive.left   = [ [ s:light_gray, s:true_black ], [ s:light_gray, s:true_black ] ]
let s:p.inactive.middle = [ [ s:light_gray, s:true_black ] ]
let s:p.inactive.right  = [ [ s:light_gray, s:true_black ], [ s:light_gray, s:true_black ] ]
let s:p.insert.left     = [ [ s:black, s:blue ], [ s:white, s:gray ] ]
let s:p.insert.middle   = [ [ s:white, s:dark_gray ] ]
let s:p.insert.right    = [ [ s:black, s:blue ], [ s:blue, s:gray ] ]
let s:p.replace.left    = [ [ s:black, s:green ], [ s:white, s:gray ] ]
let s:p.replace.middle  = [ [ s:white, s:dark_gray ] ]
let s:p.replace.right   = [ [ s:black, s:green ], [ s:green, s:gray ] ]
let s:p.visual.left     = [ [ s:black, s:orange ], [ s:white, s:gray ] ]
let s:p.visual.middle   = [ [ s:white, s:dark_gray ] ]
let s:p.visual.right    = [ [ s:black, s:orange ], [ s:orange, s:gray ] ]
let s:p.tabline.left    = [ [ s:white, s:gray ] ]
let s:p.tabline.tabsel  = [ [ s:black, s:sand ] ]
let s:p.tabline.middle  = [ [ s:white, s:gray ] ]
let s:p.tabline.right   = [ [ s:white, s:dark_gray ] ]

let g:lightline#colorscheme#kg8m#palette = lightline#colorscheme#flatten(s:p)
